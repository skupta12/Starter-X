...
const ManagementPage = () => {
  ...
  const managementPosts = blogs.filter((post) => post.label === 'Management').slice(0, 5);
      ...
      <MaxWidthWrapper>
        <div>
          <div className="text-center">
            <h1 className={`${styles.heading1} mb-4`}>Management posts</h1>
            <p className={`${styles.paragraph} mx-auto max-w-[50ch]`}>
              Suspendisse erat eget aliquam eget sollicitudin ac at purus ac orci sit cras tortor a in. Vitae aliquam
              in accumsan lacus tincidunt bibendum purus faucibus feugiat orci.
            </p>
          </div>
          <ActiveCategory className='mt-10 mb-20' />
          <div className="grid lg:grid-cols-2 grid-cols-1">
            {managementPosts.map((item, i) => (
              <Link key={i}
               className={`
                ${i < 2 ? "border-b-0" : ""}
