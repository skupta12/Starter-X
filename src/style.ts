const styles = {
    boxWidth: "mx-auto w-full max-w-[1360px] px-4 md:px-20 relative",
  
    heading2: "lg:text-[56px] md:text-[48px] leading-[1.2] sm:text-[36px] text-[32px] font-medium",
    heading3: "lg:text-[32px] md:text-[28px] sm:text-[24px] text-[22px] font-medium leading-[1]",
 
    paragraph: "font-normal text-primary-200 sm:text-[18px] text-base leading-[1.6]",
  
    flexCenter: "flex justify-center items-center",
    flexStart: "flex justify-center items-start",
    flexBetween: "flex justify-between items-center",
  
    sectionPadding: "lg:py-32 py-16",
    // routePadding: "lg:pt-[180px] pt-[150px]",
  
    // marginX: "sm:mx-16 mx-6",
    // marginY: "sm:my-16 my-6",
  };
  
export default styles;
